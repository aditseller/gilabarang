...
<?= Html::a('<span class="glyphicon glyphicon-camera"></span> Ganti Foto',['changepp'],['class'=>'btn btn-default btn-block']);  ?>
...
<?= Html::a('<span class="glyphicon glyphicon-edit"></span> Ubah Password',['changepassword']);  ?>
  
  <hr/>
  <?= Html::a('<span class="glyphicon glyphicon-edit"></span> Edit Profil',['editprofile'],['class'=>'btn btn-primary btn-block']);  ?>
  
  <?= Html::a('<span class="glyphicon glyphicon-comment"></span> Lihat Umpan Balik',['feedback'],['class'=>'btn btn-info btn-block']);  ?>
 ...
 <?= Html::a('<span class="glyphicon glyphicon-edit"></span> Edit Kontak',['editcontact'],['class'=>'btn btn-warning btn-block']);  ?>
 </div>
 </div>
  ...
  <?= Html::a('<span class="glyphicon glyphicon-plus"></span> Pasang Iklan',['product/add'],['class'=>'btn btn-success ']);  ?>
</div>
</div>
<ul class="nav nav-pills" role="tablist">
  <li role="presentation" class="active"><a><span class="badge">
  
  <?php echo Product::find()->where(['id_seller' => Yii::$app->user->identity->id])->count(); ?>
...
<div class="panel-body" id="yw7">
		...
		$productUserLogin = Product::find()->where(['id_seller' => Yii::$app->user->identity->id])->orderBy(['start_date' =>SORT_DESC])->all();
